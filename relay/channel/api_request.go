...
func SetupApiRequestHeader(info *common.RelayInfo, c *gin.Context, req *http.Header) {
	if service.IsExternalBilling(c) {
		req.Set("X-Panstar-Request-Id", c.GetString(common2.RequestIdKey))
	}
...
const (
	...
	panstarStateOwnerHeaderLower   = "x-panstar-state-owner"
)
...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// Private Gateway ownership never leaves New API.
	panstarStateOwnerHeaderLower: {},
...
func applyHeaderOverridePlaceholders(template string, c *gin.Context, apiKey string) (string, bool, error) {
	...
	if strings.HasPrefix(trimmed, clientHeaderPlaceholderPrefix) {
		...
		if strings.EqualFold(name, panstarStateOwnerHeaderLower) {
			return "", false, nil
		}
...
func processHeaderOverride(info *common.RelayInfo, c *gin.Context) (map[string]string, error) {
	...
	for k, v := range headerOverrideSource {
		...
		if key == panstarStateOwnerHeaderLower {
			continue
		}
